...
const makeMessage = (...states) => ({ publicFiles: states.map(makeFile) });
...
describe('messageHasPendingFiles(message)', () => {
  it('is falsy when publicFiles property is absent', () => {
    expect(messageHasPendingFiles({})).toBeFalsy();
  });

  it('returns false for an empty publicFiles array', () => {
    expect(messageHasPendingFiles({ publicFiles: [] })).toBe(false);
  });
...
describe('messageHasFailedFiles(message)', () => {
  ...
  it('returns false for an empty publicFiles array', () => {
    expect(messageHasFailedFiles({ publicFiles: [] })).toBe(false);
  });
